...
public class Jeu {
	...
	public void testerCase(Case mCase, Pion pion, Joueur joueur) {
		...
		if(mCase instanceof CaseDepart) {
			mCase.ajouterArgent(joueur, ((CaseDepart) mCase).getSomme());
		} else if(mCase instanceof CaseImpot) {
			mCase.retirerArgent(joueur, ((CaseImpot) mCase).getSomme());
		} else if(mCase instanceof CaseChance) {
			mCase.ajouterArgent(joueur, ((CaseChance) mCase).getSomme());
			...
			if(proprio == null) {
				...
				if(reponse == 'o') {
					//On met le nouveau proprio et on enlève l'argent
					((CaseNormal) mCase).setPropriétaire(joueur);
					mCase.retirerArgent(joueur, ((CaseNormal) mCase).getPrix());
				...
				//Sinon le joueur sur cette case paye le loyer à l'autre joueur
				mCase.retirerArgent(joueur, ((CaseNormal) mCase).getLoyer());
				mCase.ajouterArgent(proprio, ((CaseNormal) mCase).getLoyer());
			}
		}
	}
